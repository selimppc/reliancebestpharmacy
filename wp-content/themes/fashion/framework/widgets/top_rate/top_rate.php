    ...
    class WPO_Top_Rate_Widget extends WPO_Widget {
        public function __construct() {
            parent::__construct(
                ...
                __('WPO Top Rate Widget', 'fashion'),
                // Widget description
                array( 'description' => __( 'The highest rated posts on your site', 'fashion' ), )
            );
        ...
        public function widget( $args, $instance ) {
            ...
            $title      = apply_filters('widget_title', empty($instance['title']) ? __('Top Rated', 'fashion') : $instance['title'], $instance, $this->id_base);
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo $this->get_field_id('post_type'); ?>"><?php _e('Get most rated:', 'fashion'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('sort'); ?>"><?php _e('Sort by:', 'fashion'); ?></label>
                <select class="widefat" id="<?php echo $this->get_field_id('sort'); ?>" name="<?php echo $this->get_field_name('sort'); ?>">
                <option <?php selected('bayesian_rating', $instance['sort']); ?> value="bayesian_rating"><?php _e('Overall bayesian rating (score)', 'fashion'); ?></option>
                <option <?php selected('rating', $instance['sort']); ?> value="rating"><?php _e('Average rating', 'fashion'); ?></option>
                <option <?php selected('votes', $instance['sort']); ?> value="votes"><?php _e('Number of votes', 'fashion'); ?></option>
                </select>
            </p>
            ...
            <p>
                ...
                <?php _e('Ascending', 'fashion'); ?>
                </label>
            </p>

            <p>
                <label for="<?php echo $this->get_field_id('date_limit'); ?>"><?php _e('Ignore posts older than:', 'fashion'); ?></label>
                <input id="<?php echo $this->get_field_id('date_limit'); ?>" name="<?php echo $this->get_field_name('date_limit'); ?>" type="text" value="<?php echo $instance['date_limit']; ?>" size="3" /> <?php _e('days', 'fashion'); ?>
                <br />
                <small><?php _e('(0 to ignore date)', 'fashion'); ?></small>
            </p>

            <p>
                <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Limit:', 'fashion'); ?></label>
