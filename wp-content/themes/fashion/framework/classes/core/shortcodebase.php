...
abstract class WPO_Shortcode_Base{
	...
	?>
			...
			<button type="button" class="btn btn-primary wpo-button-save"><?php esc_html_e('Save','fashion'); ?></button>
			<button type="button" class="btn btn-default wpo-button-back"><?php esc_html_e('Back to list','fashion'); ?></button>
	...
	private function renderUploader( $_id, $_name ) {
		...
		$output .= '<input id="' . $id . '" class="upload" type="text" name="'.$name.'" placeholder="' . __('No file chosen', 'fashion') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Upload', 'fashion' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'Remove', 'fashion' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'Upgrade your version of WordPress for full media support.', 'fashion' ) . '</i></p>';
		}
